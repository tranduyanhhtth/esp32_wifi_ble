...
static void gatts_profile_a_event_handler(esp_gatts_cb_event_t event, esp_gatt_if_t gatts_if, esp_ble_gatts_cb_param_t *param)
{
    ...
    case ESP_GATTS_READ_EVT:
    {
        ...
        const uint8_t message[] = {
            0x53, 0x65, 0x6E, 0x64, 0x20, 0x75, 0x72, 0x20, 0x28, 0x73, 0x73, 0x69, 0x64, 0x26, 0x70, 0x61, 0x73, 0x73, 0x29};
        size_t message_len = sizeof(message);

        if (message_len > sizeof(rsp.attr_value.value))
        {
            message_len = sizeof(rsp.attr_value.value);
        }
    ...
    case ESP_GATTS_WRITE_EVT:
    {
        ...
        if (!param->write.is_prep)
        {
            ...
            ble_data_recv_handle(param->write.value, param->write.len); // function pointer
    ...
    case ESP_GATTS_STOP_EVT:
        break;
    case ESP_GATTS_CONNECT_EVT: /***************Config and Connect Ble***************/
    {
...
{
    esp_ble_gatts_send_indicate(gl_profile_tab[PROFILE_A_APP_ID].gatts_if, gl_profile_tab[PROFILE_A_APP_ID].conn_id, gl_profile_tab[PROFILE_A_APP_ID].char_handle, length, data, true);
}

void app_ble_data_recv_callback(uint8_t *data, uint16_t length)
{
    esp_log_buffer_char(ESP_TAG, data, length);
    /******************* Connect to Wifi ********************/
    ...
    ESP_LOGI(ESP_TAG, "SSID: %s", ssid);
    ESP_LOGI(ESP_TAG, "Password: %s", password);

    wifi_init_station(ssid, password);
    /********************************************************/

    const char *message_str = "Send success";
    size_t message_len = strlen(message_str);
    uint8_t *message = (uint8_t *)malloc(message_len);
    if (message == NULL)
    {
        ESP_LOGE(ESP_TAG, "Failed to allocate memory for message");
        return;
    }
    memcpy(message, message_str, message_len);
    app_ble_send_data(message, message_len);
    free(message);
}
...
void app_ble_set_data_recv_callback(void *callback)
{
    if (callback)
    {
        ble_data_recv_handle = (ble_data_recv_handle_t)callback;
    }
}
